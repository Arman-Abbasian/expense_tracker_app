...
export const useCostActions = () => {
  ...
  const initialLoading = () => {
    setAllCosts({
      ...allcosts,
      costs: { cost: [], loading: true, error: null },
    });
    axios
      ...
      .then((res) => {
        ...
        setAllCosts({
          ...allcosts,
          costs: { cost: filteredValues, loading: false, error: null },
        });
      ...
      .catch((err) =>
        setAllCosts({
          ...allcosts,
          costs: { cost: [], loading: false, error: err.message },
        })
      );
  };
  ...
  const addOneCost = (payload) => {
    axios
      .post(`http://localhost:4000/expenses`, payload)
      .then((res) => {
  ...
  const deleteOneCost = (payload) => {
    ...
    axios
      .delete(`http://localhost:4000/expenses/${payload.id}`)
      .then((res) => {
  ...
  //change filter to initial state
  const resetFilters = () => {
    setAllCosts({
      ...allcosts,
      filters: { name: "", costRange: [0, 0], kind: "" },
